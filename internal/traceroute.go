...
func TraceRoute(rest_of_command []string) {

	ipRegex := "(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)"
	ipAddress := regexp.MustCompile(ipRegex)

	timeRegex := ` ([0-9]+(?:\.[0-9]+)?)\s*ms`
	timeAddress := regexp.MustCompile(timeRegex)

	cmd := exec.Command("traceroute", rest_of_command...)
	...
	if ErrRunningCommand != nil {
		aphrodite.PrintError("error running command: traceroute: " + ErrRunningCommand.Error() + "\n")
		aphrodite.PrintError(stderr.String())
		return
	}

	if stderr.Len() > 0 {
		fmt.Println("error: from the command output: ", stderr.String())
	}
	...
	for scanner.Scan() {
		...
		if len(matches) > 1 {
			for _, timeValue := range matches {
				...
				number_of_seconds, _ := strconv.Atoi(strings.TrimSpace(strings.Split(timeValue, ".")[0]))
